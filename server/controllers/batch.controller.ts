...
export async function uploadBatchExcel(req: Request, res: Response) {
  try {
    ...
    // Guard: batch Excel is not supported for templates with table (row_group) or chorus block fields
    const tableFields = templateFields.filter(f => f.fieldType === 'row_group' || f.fieldType === 'block');
    if (tableFields.length > 0) {
      const hasBlocks = templateFields.some(f => f.fieldType === 'block');
      return res.status(400).json({
        message: hasBlocks
          ? 'Batch Excel upload is not supported for templates that contain chorus block sections'
          : 'Batch Excel upload is not supported for templates that contain table fields',
        tableFields: tableFields.map(f => f.name),
        hint: hasBlocks
          ? 'Templates with {%#BLOCKNAME%}...{%/BLOCKNAME%} sections must be filled individually'
          : 'Templates with <<TABLE_NAME>> markers must be filled individually using the table data entry interface',
      });
    }
...
export async function getBatchSessionInfo(req: Request, res: Response) {
  try {
    ...
    const session = await getBatchSession(sessionUuid);
 ...
 * PUT /api/batch/documents/:documentUuid/status
 */
export async function updateDocumentStatus(req: Request, res: Response) {
  try {
    ...
    const updated = await updateBatchDocumentStatus(documentUuid, status);
 ...
 * POST /api/batch/:uuid/create-documents
 */
export async function createDocumentsFromBatch(req: Request, res: Response) {
  try {
    const sessionUuid = req.params.uuid;
    
    console.log('Creating documents from batch session:', sessionUuid);
    
    // Get approved documents
    const approvedDocs = await getApprovedDocuments(sessionUuid);
...
/**
 * Approve all pending documents in batch session
 * POST /api/batch/:uuid/approve-all
 */
export async function approveAllDocuments(req: Request, res: Response) {
  try {
    const sessionUuid = req.params.uuid;
    
    console.log('Approving all documents in batch session:', sessionUuid);
    ...
    const pendingDocs = session.batchDocuments.filter(doc => doc.status === 'pending');
    ...
    for (const doc of pendingDocs) {
      try {
        await updateBatchDocumentStatus(doc.uuid, 'approved');
        results.push({ documentUuid: doc.uuid, success: true });
      } catch (error) {
        results.push({ 
    ...
    console.log(`Approved ${successCount}/${pendingDocs.length} documents`);
    
    return res.status(200).json({
      success: true,
      message: `Approved ${successCount} documents`,
    ...
    return res.status(500).json({
      message: 'Failed to approve all documents',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
}
 ...
 * PUT /api/batch/:uuid/bulk-status
 */
...
export async function downloadBatchDocuments(req: Request, res: Response) {
  try {
    ...
    // Add documents to archive with sanitized filenames (supports Vietnamese)
    for (const doc of documents) {
      const filePath = path.resolve(doc.filePath);
      
      if (fs.existsSync(filePath)) {
        const fileName = `${sanitizeFilename(doc.name)}.docx`;
