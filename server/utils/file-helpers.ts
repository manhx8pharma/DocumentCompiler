...
// Returns array of unique placeholders in order of first occurrence
...
// Extract placeholders with their positions (0-indexed order of first occurrence)
export interface PlaceholderWithPosition {
  name: string;
  position: number;
}

export const extractPlaceholdersWithPositions = (content: string): PlaceholderWithPosition[] => {
  const placeholders = extractPlaceholders(content);
  return placeholders.map((name, index) => ({
    name,
    position: index
  }));
};
