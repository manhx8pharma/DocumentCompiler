...
export const simpleTemplateService = {
  async uploadTemplate(file: { originalname: string; buffer: Buffer }, data: { name: string; category: string; description?: string }) {
    try {
      ...
      // Convert placeholders to field objects with position (order of first appearance)
      const extractedFields = cleanPlaceholders.map((placeholder: string, index: number) => ({
        name: placeholder,
        displayName: placeholder.charAt(0).toUpperCase() + placeholder.slice(1),
        fieldType: 'text',
        required: false,
        position: index, // Position based on order of first appearance in document
      }));
