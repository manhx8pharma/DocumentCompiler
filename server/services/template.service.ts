...
import { getEnhancedMammothOptions, getPreviewStyles } from './template-processor.service';
...
export const templateService: TemplateService = {
  ...
  async getTemplatePreviewHtml(templateUuid) {
    try {
      ...
      // Convert to HTML with mammoth using enhanced options
      const result = await mammoth.convertToHtml({ buffer: fileContent }, getEnhancedMammothOptions());
