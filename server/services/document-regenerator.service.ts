...
import { processTemplateWithData, getEnhancedMammothOptions, getPreviewStyles } from './template-processor.service';
...
export async function getDocumentPreview(documentUuid: string): Promise<string> {
  try {
    ...
    if (document.filePath && fs.existsSync(document.filePath)) {
      ...
      const result = await mammoth.convertToHtml({ buffer }, getEnhancedMammothOptions());
