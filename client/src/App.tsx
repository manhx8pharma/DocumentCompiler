...
import DocumentUpdatePage from "./pages/document-update";
import DocumentsPage from "./pages/documents";
import TemplatesPage from "./pages/templates";
import TemplatePreviewPage from "./pages/template-preview";
import TableColumnEditorPage from "./pages/table-column-editor";
import TableDataPage from "./pages/table-data-page";
import BlockDataPage from "./pages/block-data-page";
...
function TestDocuments() {
  ...
  const { data: documentsResponse, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (searchTerm.trim()) {
        params.append('searchQuery', searchTerm.trim());
      }
...
function DocumentUpdate() {
  ...
  const updatePreview = async () => {
    // Support both nested template.uuid and flat templateUuid formats
    const templateUuidValue = document?.template?.uuid || document?.templateUuid;
    if (!templateUuidValue) return;
    ...
    try {
      ...
      const response = await fetch('/api/documents/preview-template', {
        ...
        body: JSON.stringify({
          templateUuid: templateUuidValue,
      ...
      if (response.ok) {
        ...
        // API returns previewHtml field in response
        const htmlContent = data.previewHtml || data.html || data.preview || '';
  ...
  useEffect(() => {
    const templateUuidValue = document?.template?.uuid || document?.templateUuid;
    if (templateUuidValue && fieldsData && fieldsData.length > 0) {
      updatePreview();
    }
  }, [fieldValues, documentName, document?.template?.uuid, document?.templateUuid, fieldsData]);
...
function BulkDownload() {
  ...
  const handleExcelDownload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/documents/bulk-download/excel', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      let filename = 'documents-export.xlsx';
              ...
              <div>
                    ...
                    <div className="space-y-1 max-h-32 overflow-y-auto text-sm">
                      {previewData.documents.slice(0, 5).map((doc: any) => (
                        <div key={doc.uuid} className="flex justify-between items-center py-1">
                          ...
                          <span className={`text-xs px-2 py-1 rounded ${doc.canGenerate ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                            {doc.canGenerate ? 'Available' : 'Missing'}
                          </span>
                        </div>
                      ))}
                ...
                <div className="space-y-2">
                  <button
                    ...
                    disabled={isDownloading || previewData.totalDocuments === 0 || (previewData.canGenerate || 0) === 0}
                    className="w-full bg-green-600 text-white py-2 px-4 rounded hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isDownloading ? 'Preparing Download...' : 
                     previewData.totalDocuments === 0 ? 'No Documents Found' :
                     (previewData.canGenerate || 0) === 0 ? 'No Valid Files Found' :
                     `Download ${previewData.canGenerate || previewData.totalDocuments} Documents`}
                  </button>
                  <button
                    onClick={handleExcelDownload}
                    disabled={isDownloadingExcel || previewData.totalDocuments === 0}
                  ...
                  >
                    ...
                    {isDownloadingExcel ? 'Exporting Excel...' : 
                     previewData.totalDocuments === 0 ? 'No Documents Found' :
                     `Download Excel (${Object.keys(previewData.groupedByTemplate || {}).length} files)`}
                  </button>
                </div>
              </div>
            )}
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="bg-white p-6 rounded-lg shadow border mb-6">
        ...
        <div className="mt-6">
          <TemplatePicker
            selectedTemplates={filters.templateUuids}
            onSelectionChange={(uuids) => setFilters(prev => ({ ...prev, templateUuids: uuids }))}
          />
        </div>
    ...
    <QueryClientProvider client={queryClient}>
      <div className="min-h-screen bg-gray-50">
        ...
        <Switch>
          ...
          <Route path="/document-preview/:id">
            <DocumentPreviewPage />
          </Route>
          <Route path="/document-update/:documentUuid">
            <DocumentUpdatePage />
          </Route>
          <Route path="/template/:uuid/table/:name">
            <TableColumnEditorPage />
          </Route>
          <Route path="/document/:documentUuid/table/:tableName">
            <TableDataPage />
          </Route>
          <Route path="/document/:documentUuid/block/:blockName">
            <BlockDataPage />
          </Route>
